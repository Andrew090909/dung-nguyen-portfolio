(() => {
  ...
  function render(data) {
    ...
    root.innerHTML = data.cases.map((item, index) => {
      ...
      return `<section class="case reveal in" id="${escapeHtml(item.id)}"><div class="container"><div class="case-head"><div class="case-label"><span class="case-no">0${index + 1} / ${escapeHtml(item.year)}</span><h2>${escapeHtml(text.title)}</h2><span class="pill">${escapeHtml(text.industry)}</span></div><div class="case-meta"><div><small>${labels.context}</small><p>${escapeHtml(text.context)}</p></div><div><small>${labels.role}</small><p>${escapeHtml(text.role)}</p></div></div></div><div class="case-copy"><h3>${labels.problem}</h3><p>${escapeHtml(text.problem)}</p></div><div class="case-gallery">${item.images.map((image, imageIndex) => { const src = String(image).startsWith('/') ? image : imageBase + encodeURIComponent(image); return `<figure><img loading="lazy" src="${escapeHtml(src)}" alt="${escapeHtml(text.title)} — ${imageIndex + 1}"></figure>`; }).join('')}</div><div class="case-copy"><h3>${labels.approach}</h3><div><p>${escapeHtml(text.approach)}</p><h3 style="font-size:18px;margin:28px 0 12px">${labels.deliverables}</h3><ul>${text.deliverables.map(value => `<li>${escapeHtml(value)}</li>`).join('')}</ul></div></div></div></section>`;
    }).join('');
